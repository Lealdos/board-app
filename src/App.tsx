...
import { boardFileName, boardTitle, downloadFile, exportStandaloneHtml } from '@/lib/export-html'
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (parsed.events.length === 0) {
        setError(`No events found in "${file.name}". Is it an RBL report?`)
        return
      }
      setModel(parsed)
      toast.success(`Loaded ${parsed.events.length} events from ${file.name}`)
    } catch (cause) {
      console.error(cause)
      ...
      <header className="app-chrome sticky top-0 z-10 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1800px] items-center gap-4 px-6 py-3">
          <div className="min-w-0">
            <h1 className="truncate text-lg font-semibold">Elevator Board</h1>
          ...
          {model ? (
            <div className="ml-auto flex items-center gap-2">
              ...
              {hiddenCount > 0 ? <Badge variant="outline">{hiddenCount} hidden</Badge> : null}
              {layout ? (
                <Badge variant="outline">
                  {layout.sheets.length === 1 ? '1 sheet' : `${layout.sheets.length} sheets`} ·{' '}
                  {Math.round(layout.scale * 100)}%
                </Badge>
              ) : null}
              <Separator orientation="vertical" className="mx-1 h-6" />
              <Button variant="outline" size="sm" onClick={handleExport} disabled={!layout}>
                <Download className="size-4" />
                Download HTML
              </Button>
              <Button size="sm" onClick={() => window.print()} disabled={!layout}>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <div className="app-chrome space-y-4">
              {model.warnings.length > 0 ? (
                <Alert>
                  <AlertTriangle className="size-4" />
                  <AlertTitle>Check these before printing</AlertTitle>
                  <AlertDescription>
              ...
              {layout?.overflowed ? (
                <Alert>
                  <AlertTriangle className="size-4" />
                  <AlertTitle>Split across {layout.sheets.length} sheets</AlertTitle>
                  <AlertDescription>
                    Too many events to stay readable on one page. Hide rows to pull it back to one
                    sheet.
                  </AlertDescription>
                </Alert>
              ) : null}
            ...
            <div ref={paneRef} className="board-pane">
              <div
                ...
                style={{
                  height: layout ? `${layout.sheets.length * 1056 * previewScale + 32}px` : undefined,
                }}
              >
                <div
                  className="board-scaler"
                  style={{ transform: `scale(${previewScale})`, width: SHEET_WIDTH_PX }}
                >
                  <BoardPreview model={model} layout={layout} measureRef={measureRef} />
                </div>
              </div>
            </div>
          </div>
